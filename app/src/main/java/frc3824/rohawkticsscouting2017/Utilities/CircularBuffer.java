...
public class CircularBuffer {
    ...
    public void insert(String item) {
        data[insertLocation] = String.format("<font color='black'>%s</font><br>\n", item);
     ...
     *      - the item to be inserted
     */
    public void insert(String item, String color) {
        data[insertLocation] = String.format("<font color='%s'>%s</font><br>\n", color, item);
        insertLocation = (insertLocation + 1) % data.length;
        /**
         * If the queue is full, this means we just overwrote the front of the
         * queue. So increment the front location.
