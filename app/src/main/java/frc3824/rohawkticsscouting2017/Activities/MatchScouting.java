...
public class MatchScouting extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        database = Database.getInstance(eventKey);
        ...
        if (match_number > 0)
        {
            if(allianceColor.equals(Constants.Alliance_Colors.BLUE))
            {
                ...
                {
                    case 1:
                        team_number = database.getMatch(match_number).blue1;
                        break;
                    case 2:
                        team_number = database.getMatch(match_number).blue2;
                        break;
                    case 3:
                        team_number = database.getMatch(match_number).blue3;
                        break;
                    default:
                        assert false;
                }
            ...
            else
            {
                ...
                {
                    case 1:
                        team_number = database.getMatch(match_number).red1;
                        break;
                    case 2:
                        team_number = database.getMatch(match_number).red2;
                        break;
                    case 3:
                        team_number = database.getMatch(match_number).red3;
                        break;
                    default:
                        assert false;
                }
        ...
        TeamInMatch tim = database.getTeamInMatch(match_number, team_number);
        if(tim != null) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if(practice || match_number == 1)
        {
            menu.removeItem(R.id.previous_match);
        }
        if(practice || match_number == database.getNumberOfMatches())
        {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (error.equals("")) {
                    ...
                    if (practice) {
                        ...
                        intent.putExtra(Constants.Intent_Extras.MATCH_NUMBER, match_number - 1);
                    }
                    startActivity(intent);
                } else {
                    Toast.makeText(MatchScouting.this, String.format("Error: %s", error), Toast.LENGTH_LONG).show();
